...
import { renderGameToText as debugRenderGameToText } from './debug/renderGameToText';
import { wrapPointInPlace, randomPointInOctagon } from './geometry/octagon';
...
import type { AudioEvent, AsteroidState, BulletState, GameMode, GameState, UfoState, UfoType, Vec2 } from './types';
...
export class Game {
  ...
  constructor(private readonly canvas: HTMLCanvasElement, private readonly options: GameOptions = {}) {
    this.renderer = new CanvasRenderer(canvas, GAME_CONFIG);
  ...
  renderGameToText(): string {
    return debugRenderGameToText(this.state);
  }

  getDebugState(): GameState {
  ...
  private update = (dt: number): void => {
    ...
    updateAsteroids(state.asteroids, state.octagon, dt);
    updateBullets(state.playerBullets, state.octagon, dt);
    updateBullets(state.ufoBullets, state.octagon, dt);
  ...
  private render = (): void => {
    this.renderer.renderWithState(this.state);
  };
  ...
  private handlePlayerPhase(dt: number, input: ReturnType<InputManager['getState']>): void {
    ...
    if (player.alive) {
      ...
      wrapPointInPlace(player.pos, state.octagon);
    } else {
      player.thrusting = false;
    }
  }
  ...
  private handleWaveAndRespawn(dt: number): void {
    ...
    if (state.mode === 'respawning' && !state.player.alive) {
      ...
      if (state.respawnTimer <= 0) {
        if (this.isSpawnSafe({ x: 0, y: 0 }, GAME_CONFIG.player.collisionRadius + 16)) {
          this.respawnPlayer();
        } else {
  ...
  private updateUfo(dt: number): void {
    ...
    if (state.ufo) {
      updateUfoMovement(state.ufo, state.octagon, dt);
      ...
      if (state.ufoSpawnTimer <= 0) {
        ...
        state.ufo = createUfo(this.nextId(), type, state.octagon);
  ...
  private fireUfoBullet(ufo: UfoState): void {
    const state = this.state;
    const player = state.player;
  ...
  private tryHyperspace(): void {
    ...
    for (let i = 0; i < GAME_CONFIG.player.hyperspaceSafeAttempts; i += 1) {
      const candidate = randomSafeHyperspacePoint(state.octagon, Math.random, GAME_CONFIG.player.collisionRadius);
  ...
  private spawnWave(wave: number): void {
    const result = spawnAsteroidWave(this.state.nextEntityId, wave, this.state.octagon);
  ...
  private queueAudio(name: AudioEvent['name']): void {
    ...
    stateLogEvent(this.state, `audio:${name}`);
  }
...
function stateLogEvent(state: GameState, entry: string): void {
  state.debugLastEvents.push(entry);
  if (state.debugLastEvents.length > 16) state.debugLastEvents.shift();
}
