...
import { containsPoint, pointOnRandomEdge, randomPointInOctagon } from './geometry/octagon';
import { randChoice, randRange, randSign, randUnitVec } from './math/random';
import { add, fromAngle, normalize, scale } from './math/vector';
import type { AsteroidSize, AsteroidState, BulletState, OctagonGeometry, PlayerState, UfoState, UfoType, Vec2 } from './types';
...
export function spawnAsteroidWave(
  ...
  while (asteroids.length < count && attempts < 500) {
    attempts += 1;
    const pos = randomPointInOctagon(geometry, rng, 60);
    if (Math.hypot(pos.x, pos.y) < 110) continue;
...
export function createUfo(
  ...
  const { point, inward } = pointOnRandomEdge(geometry, rng);
...
export function randomSafeHyperspacePoint(
  ...
  for (let i = 0; i < 100; i += 1) {
    const p = randomPointInOctagon(geometry, rng, radius + 4);
    if (containsPoint(p, geometry, radius + 1)) return p;
  }
  return { x: 0, y: 0 };
}
